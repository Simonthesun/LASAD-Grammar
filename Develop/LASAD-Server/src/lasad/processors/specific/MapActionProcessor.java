...
public class MapActionProcessor extends AbstractActionObserver implements ActionObserver {
	        ...
	        switch (a.getCmd())
	        {
	        	...
	        	case CreateElementSet:
		        	...
		        	else if (eltType.equals("box"))
		        	{
		        		if (a.getParameterValue(ParameterTypes.AutoGenerated) == null)
		        		{
		        			action = "Create Box Set";
		        		}
		        		else
		        		{
		        			action = "Auto Create Box Set";
		        		}
		        		toolMsg.setAsAttempt("");
		        	}
		        	else if (eltType.equals("relation"))
		        	{
		        		if (a.getParameterValue(ParameterTypes.AutoGenerated) == null)
		        		{
		        			action = "Create Relation Set";
		        		}
		        		else
		        		{
		        			action = "Auto Create Relation Set";
		        		}
		        		toolMsg.setAsAttempt("");
		        	}
		        	else
    ...
    private boolean linkExistsBetweenParents(Vector<String> parents, int mapID)
    {
    	if (parents != null && parents.size() == 2)
    	{
	    	...
	    	for (String parentID : parents)
	    	{
				...
				else if (parentID.equalsIgnoreCase("SECOND-LAST-ID")) {
					origParents.add(myServer.currentState.secondLastTopLevelElementID);
				}
				else
				{
					try
	...
	public boolean processCreateElementSet(Action a, User u) {
		int mapID = ActionProcessor.getMapIDFromAction(a);

		Vector<String> parents = a.getParameterValues(ParameterTypes.Parent);
		String box = parents.firstElement();
		parents.remove(0);

		if (box.equalsIgnoreCase("LAST-ID")) {
			box = Integer.toString(myServer.currentState.lastTopLevelElementID);
		}

		Vector<Action> createElementActions = new Vector<Action>();

		for (String id : parents) {
			Action action = new Action(Commands.CreateElement, Categories.Map);
			action.addParameter(ParameterTypes.Type, a.getParameterValue(ParameterTypes.Type));
			action.addParameter(ParameterTypes.MapId, a.getParameterValue(ParameterTypes.MapId));
			action.addParameter(ParameterTypes.ElementId, a.getParameterValue(ParameterTypes.ElementId));
			action.addParameter(ParameterTypes.Parent, box);
			action.addParameter(ParameterTypes.Parent, id);
			action.addParameter(ParameterTypes.SiblingsAlreadyUpdated, "false");

			createElementActions.add(action);
...
//					Action b = new Action("CREATE-ELEMENT", "MAP");
		...
		for (Action action : createElementActions) {
			processCreateElement(action, u);
		}
	...
	public boolean processAction(Action a, User u, String sessionID) {
		...
		if (u != null && a.getCategory().equals(Categories.Map)) {
			...
			case CreateElement:// Check
				...
				returnValue = true;
				break;
			case CreateElementSet:// Woah
				actionIsLoggable = processCreateElementSet(a, u);
				returnValue = true;
				break;
			case UpdateElement:
			case AutoResizeTextBox:
